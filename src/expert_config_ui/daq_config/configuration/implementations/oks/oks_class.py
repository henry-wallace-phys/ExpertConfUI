...
from expert_config_ui.daq_config.configuration.interfaces.configuration_interface import (IClassObjectLifecycle,
                                                                                          IClassObjectManager,
                                                                                          IObjectModifier)

# *****************************************************************************
class OksKernelClassManager(IClassObjectManager, OksKernelInteraction):
# *****************************************************************************
    def get(self, object_class: str)->oks.OksClass:
        return self._configuration.configuration.find_class(object_class)

    def get_all(self, object_class: str | list[str] | None = None) -> list[oks.OksClass]:
        '''
        ...
        if isinstance(object_class, str):
            return [self.get(object_class)]

        return [self.get(c) for c in object_class]


# *****************************************************************************
class OksKernelClassModifier(IObjectModifier, OksKernelInteraction):
# *****************************************************************************
    '''
    For modifying classes in the OKS configuration.    
    '''
    ...
    def set(self, obj: oks.OksClass, attr_name: str, attr_value: Any) -> None:
        ...
        if attr_name not in self.__KNOWN_PROPERTIES__:
            logging.warning(f"Attribute '{attr_name}' is not a known property of OksClass.")

        if hasattr(obj, f"set_{attr_name}"):
            getattr(obj, f"set_{attr_name}")(attr_value)
        else:
            logging.error(f"Attribute '{attr_name}' does not exist in class '{obj.get_name()}'.")

# *****************************************************************************
class OksKernelClassLifecycle(IClassObjectLifecycle, OksKernelInteraction):
# *****************************************************************************    
    def __copy_class__(self, obj: oks.OksClass, name: str) -> oks.OksClass:
        cls_ = oks.OksClass(name,
                     obj.get_description(),
                     obj.get_is_abstract(),
    ...
    def create(self, object_class: str, attributes: Dict[str, Any]) -> None:
        ...
        oks.OksClass(object_class, description, is_abstract, self._configuration.configuration, transient)